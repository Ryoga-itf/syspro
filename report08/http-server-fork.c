...
extern void http_server(int portno, int ip_version, int max_connections);
...
extern int delete_zombie();

int main(int argc, char *argv[]) {
    if (!(argc == 2 || argc == 3 || argc == 4)) {
        fprintf(stderr, "Usage: %s portno {ipversion} {max_connections}\n",
                argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    if (argc == 4) {
        max_connections = strtol(argv[3], 0, 10);
    }
    http_server(portno, ip_version, max_connections);
}

void http_server(int portno, int ip_version, int max_connections) {
    ...
    int child_num = 0;
    while (1) {
        child_num -= delete_zombie();

        printf("[%d] number of child proc: %d\n", getpid(), child_num);
        if (max_connections > 0 && child_num >= max_connections) {
            printf("[%d] waiting other process.\n", getpid());
            fflush(stdout);
            if (wait(NULL) < 0) {
                perror("waitpid");
                continue;
